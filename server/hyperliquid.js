...
class HyperliquidSource {
  constructor(emit) {
    ...
    this.coins = [];
    this.subscribedTradeCoins = new Set();
    this.subscribedBooks = new Set();
  ...
  connect() {
    ...
    this.ws.on('open', () => {
      ...
      if (this.coins.length > 0) this._subscribeAllTrades();
      if (this.activeSymbol) this._subscribeBook(this.activeSymbol);
    });

    this.ws.on('message', (raw) => {
      try { this._handleMessage(JSON.parse(raw)); } catch (e) {}
    });
    ...
    this.ws.on('error', (err) => console.error('[HL] WebSocket error:', err.message));
  }
  ...
  _subscribeAllTrades() {
    ...
    if (count > 0) console.log(`[HL] Subscribed to trades for ${count} coins (total: ${this.subscribedTradeCoins.size})`);
  }
  ...
  _subscribeBook(coin) {
    ...
    for (const prev of this.subscribedBooks) {
      if (prev !== coin) {
        try { this.ws.send(JSON.stringify({ method: 'unsubscribe', subscription: { type: 'l2Book', coin: prev } })); } catch (e) {}
  ...
  _handleMessage(msg) {
    if (msg.channel === 'trades' && msg.data) {
      for (const t of msg.data) {
        ...
        this.tradesBySymbol.set(t.coin, (this.tradesBySymbol.get(t.coin) || 0) + 1);
    ...
    if (msg.channel === 'l2Book' && msg.data) {
      ...
      this.emit('book', { source: 'hyperliquid', symbol: msg.data.coin, levels: msg.data.levels, time: Date.now() });
    }
  }
      ...
      res.on('end', () => {
        try {
          ...
          if (info.universe) {
            ...
            if (this.connected && this.subscribedTradeCoins.size === 0) this._subscribeAllTrades();
          }
        } catch (e) {}
      });
    });
